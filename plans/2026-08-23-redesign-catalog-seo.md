...
| 5 | JS: sort mới (featured → bestseller/trending → updatedAt); NEW tự tính 30 ngày; filter chip `style`/`event`; card tên + sub-label; detail chip variant; showcase khách từ `wedding/config.js` | `data.js` helpers, `products.js`, `product-detail.js`, `main.js` |
